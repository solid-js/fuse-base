...
export class HMR {
  ...
  hmrUpdate({type, path, content, dependants}) {
    ...
    if (type === 'css') {
      ...
      if (!FuseBox.exists(path) && !FuseBox.exists('default/' + path))
        return false;
    ...
    // IMPORTANT : We advise to use solid-fuse.config.js > generateCSSFiles = 'quantum' or false
      ...
      for (let i = 0; i < linkTags.length; i++) {
        ...
        if (currentLinkTag.getAttribute('rel').toLowerCase() !== 'stylesheet')
          continue;
  ...
  registerStatefulComponent(pComponentClass: any) {
    ...
    if (!(componentName in this._statefulComponentsStates)) {
      console.info(
        `%cRegistered stateful component for HMR ${componentName}`,
        'color: #237abe'
      );
    }
