...
// Load config
const imageminConfig = require('../solid-imagemin.config');
...
module.exports = {
	 ...
	 * @return {Promise<any>}
	 */
	optimizeFiles : (pInputFiles, pOutputFolder, pPngConfig, pAddDotMinAndDoNotOverride, pVerbose = true) => new Promise( (resolve) =>
	{
		...
		.then( files =>
		{
			...
			pVerbose && files.map( file =>
			{
				...
				console.log(`	✓ Image ${ path.basename( file.path ) } optimized with ${customConfigText}.`.grey);
			});
	...
	run: () =>
	{
		...
		console.log(`  → Optimizing ${ totalImages } image${ totalImages > 1 ? 's' : ''} ...`.cyan);
